...
######################################################################################################################
# Hopefully replaces windows for quartz when working on a Mac

if(.Platform$OS.type=="unix") { 
  windows<-function() quartz() 
} 
...
# First code in the case when someone selects two groups and put line.high.lr above all datapoints
...
# make figure Mo interference
	...
	} else { vec.lim.b <- NA; vec.lim.c <- NA}
...
# Create table for data head(data.standadd)
			...
			colnames(data.standadd) <- c("parameter",nb,vec.cd)
...
fChecks.balances <- function(ck, sa.sf, st.el, blank, ref.mat, mor, coding, sample.naming, name.run, mainDir, use.Blank, Seafast.conc.factor, outliers.boxplot,resolution.figures, cex, cex.lab, cex.axis, Standard.element.correction,Mo.correction.Cd,...){
...
print("Calculation of recoveries: completed")
